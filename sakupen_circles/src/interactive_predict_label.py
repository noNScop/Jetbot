...
def find_model_path(provided=None):
    ...
    candidates = [
        os.path.join('src', 'best_steering_model_xy.pth'),
        os.path.join('models', 'best_steering_model_xy.pth'),
        os.path.join('src', 'best_model.pth'),
    ]
    for c in candidates:
        if os.path.exists(c):
            return c
    # last resort: first .pth in repo models folder
    found = glob.glob(os.path.join('models', '**', '*.pth'), recursive=True)
